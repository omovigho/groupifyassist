...
const ForgotPasswordPage = () => {
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.post('/user/forgot-password/start', { email });
  ...
  return (
    <div className="min-h-screen dark-navy-bg relative overflow-hidden">
      ...
      <header className="relative z-10 px-6 pt-10 text-center">
        <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-white">Forgot password</h1>
        <p className="mt-3 text-gray-300 max-w-2xl mx-auto">Enter your registered email below.</p>
      </header>
        ...
        <div className="w-full max-w-md">
          <div className="glass-morphism rounded-2xl p-6 md:p-8">
            <form onSubmit={onSubmit} className="space-y-5">
              ...
              <Button type="submit" className="w-full h-12 bg-white text-slate-900 hover:bg-gray-100 rounded-lg font-semibold button-hover-glow">Continue</Button>
              <div className="text-center text-sm text-gray-400">
                Remembered? <Link to="/login" className="text-blue-400 hover:text-blue-300 underline underline-offset-4">Back to login</Link>
              </div>
            </form>
          </div>
        </div>
