...
const Groups = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const params = new URLSearchParams({ session_type: 'group', limit: String(limit), offset: String(page*limit) });
        if (q) params.set('q', q);
    ...
    <Layout>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link to="/dashboard/groups/create" className="rounded-md bg-[#0A2540] text-white text-sm px-3 py-2">+ New Grouping</Link>
        </div>
      </div>
        ...
        <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
          <thead className="bg-slate-50 dark:bg-slate-900">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400">Group created</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-200 dark:divide-slate-800 bg-white dark:bg-slate-900">
            {rows.length === 0 ? (
              <tr><td colSpan={6} className="px-4 py-6 text-sm text-slate-500 dark:text-slate-400">No group projects yet.</td></tr>
            ) : rows.map((r)=> (
              <tr key={`g_${r.id}`}>
                ...
                <td className="px-4 py-3 text-sm">{r.group_created ?? '-'}</td>
              </tr>
            ))}
          </tbody>
        </table>
